...
if ! command -v envsubst >/dev/null 2>&1; then
  echo "claude-sharedserver: envsubst not found (install gettext)" >&2
  exit 0
fi
if ! command -v jq >/dev/null 2>&1; then
  echo "claude-sharedserver: jq not found" >&2
  exit 0
fi
...
while IFS= read -r entry; do
  ...
  if [[ "$lazy" == "true" ]]; then
    ...
    if [[ -z "$command" ]]; then
      echo "claude-sharedserver: server '$name' missing 'command' and is not lazy; skipping" >&2
      continue
    fi
    cmd_args=()
  ...
  if ! out="$("$ss_bin" "${ss_args[@]}" 2>&1)"; then
    echo "claude-sharedserver: 'sharedserver use $name' failed (exit $?):" >&2
